...
SELECT *
FROM bookings
WHERE check_in <= '2024-12-31'
  AND check_out >= '2024-01-01';
  
-- Cho biết số lượng phòng của từng loại phòng
SELECT room_type, COUNT(room_type) as total_per_type
FROM rooms
GROUP BY room_type;
...
SELECT g.guest_name, COUNT(*) as total_booking
FROM bookings b
JOIN guests g ON b.guest_id = g.guest_id
GROUP BY g.guest_name;

-- Tính doanh thu của mỗi phòng, với công thức: “Doanh thu = số ngày ở × giá thuê theo ngày”
SELECT r.room_id, r.room_type, SUM(DATEDIFF(b.check_out, b.check_in) * r.price_per_day) AS revenue
FROM bookings b
JOIN rooms r ON r.room_id = b.room_id
...
SELECT g.guest_id, g.guest_name, COUNT(*) AS number_of_bookings
FROM bookings b
JOIN guests g ON b.guest_id = g.guest_id
